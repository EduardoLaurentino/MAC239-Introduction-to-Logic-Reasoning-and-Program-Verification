...
public class CodeBreaker {
    ...
    private void strategyNew(boolean[] feedback) throws IOException {
        ...
        //adiciona cláusulas unitárias para feedbacks positivos
        for (int i = 0; i < this.nSlots; i++) {
            if (feedback[i]){
                int[] clauseU = new int[1];
                clauseU[0] = (satTradutor.attToVar(i, lastGuess[i]));
                satTradutor.addClause(clauseU);
            }
            else n++;
        }

        //adiciona cláusula conjuntiva dos feedbacks negativos
        int[] clause = new int[n];
        int m = 0;
        for (int i = 0; i < this.nSlots; i++) {
            if (!feedback[i]){
                clause[m++] = (-1 * satTradutor.attToVar(i, lastGuess[i]));
            }
        }
        satTradutor.addClause(clause);
